...
#cf = '["surface"!~"ground|dirt|earth"]'   , custom_filter=cf
...
# cols_to_norm = ['highway','cycleway', 'surface',
#                                        'maxspeed', 'lanes', 'width', 'oneway',
#                                       'centrality']
#d_frame[cols_to_norm] = d_frame[cols_to_norm].apply(lambda x: (x - x.min()) / (x.max() - x.min()))
...
if path.exists("result_cities.csv"):
    ...
    result_df = pd.DataFrame(
        columns=('place', 'average_index', 'max_index', 'min_index'))
    result_df = result_df.append(result, ignore_index=True)
    result_df.to_csv('result_cities.csv', index=False)

#d_frame.to_file("{}.geojson".format(place_name.split(',')[0]), driver='GeoJSON')
